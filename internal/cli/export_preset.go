...
func init() {
	...
	exportPresetSaveCmd.Flags().StringP("period", "p", "", "Относительный период (например: 'last month', 'this week')")
	exportPresetSaveCmd.Flags().StringP("date-from", "D", "", "Дата начала (RFC3339 или относительная)")
	exportPresetSaveCmd.Flags().StringP("date-to", "T", "", "Дата конца (RFC3339 или относительная)")
	exportPresetSaveCmd.Flags().StringP("timezone", "z", "", "Часовой пояс")
	exportPresetSaveCmd.Flags().StringP("company", "q", "", "Компания")
	exportPresetSaveCmd.Flags().StringP("solution", "S", "", "Статус решения")
	exportPresetSaveCmd.Flags().StringP("assignee", "a", "", "Исполнитель")
	exportPresetSaveCmd.Flags().StringP("search", "s", "", "Поиск")
	exportPresetSaveCmd.Flags().StringP("ticket", "t", "", "Тикет")
	exportPresetSaveCmd.Flags().BoolP("open-only", "O", false, "Только открытые")
	exportPresetSaveCmd.Flags().BoolP("closed-only", "C", false, "Только закрытые")
	exportPresetSaveCmd.Flags().BoolP("paused-only", "P", false, "Только на паузе")
	exportPresetSaveCmd.Flags().BoolP("active-only", "A", false, "Только активные")
	exportPresetSaveCmd.Flags().BoolP("all-users", "U", false, "Все пользователи")
	exportPresetSaveCmd.Flags().StringP("fields", "i", "", "Поля для экспорта (через запятую)")
	exportPresetSaveCmd.Flags().StringP("description", "d", "", "Описание пресета")
