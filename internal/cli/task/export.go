...
func init() {
	...
	ExportCmd.Flags().StringP("filename", "F", "", "Имя файла для скачивания")
	ExportCmd.Flags().StringP("timezone", "z", "", "Часовой пояс (по умолчанию: Europe/Moscow)")

	ExportCmd.Flags().StringP("period", "p", "", "Относительный период (today, week, month, 'last 7 days')")
	ExportCmd.Flags().StringP("date-from", "D", "", "Дата начала (RFC3339 или относительная)")
	ExportCmd.Flags().StringP("date-to", "T", "", "Дата конца (RFC3339 или относительная)")
	...
	ExportCmd.Flags().StringP("solution", "S", "", "Фильтр по статусу")
	...
	ExportCmd.Flags().StringP("ticket", "k", "", "Поиск по тикету")

	ExportCmd.Flags().BoolP("open-only", "O", false, "Только открытые")
	ExportCmd.Flags().BoolP("closed-only", "C", false, "Только закрытые")
	ExportCmd.Flags().BoolP("paused-only", "P", false, "Только на паузе")
	ExportCmd.Flags().BoolP("active-only", "A", false, "Только активные")
	ExportCmd.Flags().BoolP("all-users", "U", false, "Все пользователи")

	ExportCmd.Flags().StringP("fields", "L", "", "Поля через запятую")
	ExportCmd.Flags().StringP("fields-preset", "R", "", "Пресет колонок: minimal, standard, full")

	ExportCmd.Flags().BoolP("interactive", "i", false, "Интерактивный режим")
	ExportCmd.Flags().BoolP("preview", "v", false, "Preview режим")
	ExportCmd.Flags().StringP("preset", "r", "", "Имя пресета")
}
